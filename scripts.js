const init = () => {
    ...
    let isFog = false
    if (isFog) scene.fog = new THREE.FogExp2('#fff', .011)
    ...
    let plane = getPlane(60, planeMaterial); plane.name = 'main-plane';
    let boxes = getBoxGroup(12, 1, 2, boxesMaterial); boxes.name = 'main-boxes'

    planeMaterial.map = textureConcrete
    planeMaterial.bumpMap = textureConcrete
    planeMaterial.bumpScale = .05
    boxesMaterial.bumpMap = textureFinger
    boxesMaterial.bumpScale = .0005
...
const update = (renderer, scene, camera, controls, clock) => {
    ...
    boxes.children.forEach((child, index) => {
        child.scale.y = Math.abs(noise.simplex2(timeElapsed * .65 + index, timeElapsed * .5 + index)) + .001
